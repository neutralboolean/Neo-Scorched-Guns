...
public class AirGunItem extends GunItem implements IAirGun {
    ...
    public int getBarWidth(ItemStack stack) {
        int width = 0; // Fallback to 0 if not on client
        if (FMLEnvironment.dist.isClient()) {
            ...
            if (!backtanks.isEmpty()) {
                ...
                width = Math.round(13.0F * air / maxAir); // Dynamically calculate based on air level
            } else {
                width = Math.round(13.0F - (float) stack.getDamageValue() * 13.0F / (float) stack.getMaxDamage());
            }
        }
        return width;
    }
